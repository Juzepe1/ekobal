...
class TimerCountMixin(models.AbstractModel):
    _name = 'timer.count.mixin'
    _description = 'Timer Count Mixin'
    ...
    user_timer_id = fields.One2many('timer.timer.count', compute='_compute_user_timer_id',
                                    search='_search_user_timer_id', export_string_translation=False)
    ...
    def _search_is_timer_running(self, operator, value):
        ...
        running_timer_query = self.env['timer.timer.count']._search([
            ('timer_start', '!=', False),
            ('timer_pause', '=', False),
    ...
    def _search_user_timer_id(self, operator, value):
        timer_query = self.env['timer.timer.count']._search([
            ('id', operator, value),
    ...
    def _compute_user_timer_id(self):
        ...
        """
        timer_read_group = self.env['timer.timer.count']._read_group(
            domain=[
                ('user_id', '=', self.env.user.id),
                ('res_id', 'in', self.ids),
                ('res_model', '=', self._name),
    ...
    def _get_user_timers(self):
        ...
        return self.env['timer.timer.count'].search([('user_id', '=', self.env.user.id)])

    def unlink(self):
        if not self:
            return True
        timers = self.env['timer.timer.count'].search([
    ...
    def action_session_start(self):
        ...
        if not timer:
            timer = self.env['timer.timer.count'].create({
                'timer_start': False,
                'timer_pause': False,
    ...
    def action_session_stop(self):
        ...
        self.ensure_one()
        timer = self.user_timer_id
        minutes_spent = timer.action_session_stop()
        timer.unlink()
        return minutes_spent

    def action_session_pause(self):
        self.ensure_one()
        timer = self.user_timer_id
        timer.action_session_pause()

    def action_session_resume(self):
        self.ensure_one()
        self._stop_timer_in_progress()
        timer = self.user_timer_id
        timer.action_session_resume()
    ...
    def _compute_display_timer_buttons(self):
        for record in self:
            ...
            if record.timer_start:
                ...
                start_p, stop, pause, resume = True, False, False, False
            record.update({
